...
void printLogToStdOut_(ModelManager* model, const std::string& message) {
  ...
  if (sub == NULL) throw DYNError(Error::GENERAL, WrongDynamicCast);
...
void addLogEventRaw2_(ModelManager* model, const char* message1,
                      const char* message2) {
  if (!model->hasTimeline()) {
    return;
  }
...
void addLogEventRaw3_(ModelManager* model, const char* message1,
...
void addLogEventRaw4_(ModelManager* model, const char* message1,
                      const char* message2, const char* message3,
                      const char* message4) {
  if (!model->hasTimeline()) {
    return;
...
void addLogEventRaw5_(ModelManager* model, const char* message1,
                      const char* message2, const char* message3,
...
const char* modelica_integer_to_modelica_string(
    modelica_integer i, modelica_integer /*minLen*/,
    modelica_boolean /*leftJustified*/) {
...
const char* cat_modelica_string(modelica_string_const s1,
                                modelica_string_const s2) {
  std::stringstream ss("");
  ss << s1 << s2;
  return memoryManagerChars::keep(ss.str());
}
...
const char* modelica_real_to_modelica_string_format(modelica_real r,
                                                    std::string /*format*/) {
  std::stringstream ss("");
...
const char* modelica_integer_to_modelica_string_format(modelica_integer i,
                                                       std::string /*format*/) {
  std::stringstream ss("");
  ss << i;
  return memoryManagerChars::keep(ss.str());
...
const char* modelica_real_to_modelica_string(modelica_real r,
...
const char* modelica_boolean_to_modelica_string(
    modelica_boolean b, modelica_integer /*minLen*/,
    modelica_boolean /*leftJustified*/) {
  if (b) {
    return "true";
  } else {
...
const modelica_integer* integerArrayElementAddress1_(
    const modelica_integer* source, int dim1) {
...
void callExternalAutomatonModel(const std::string& modelName,
                                const char* command, const double time,
                                const double* inputs, const char** inputsName,
                                const int nbInputs, const int nbMaxInputs,
                                double* outputs, const char** outputsName,
                                const int nbOutputs, const int nbMaxOutputs,
                                const std::string& workingDirectory) {
  if (nbInputs >= nbMaxInputs)
    throw DYNError(Error::GENERAL, AutomatonMaximumInputSizeReached, modelName,
                   boost::lexical_cast<std::string>(nbInputs),
                   boost::lexical_cast<std::string>(nbMaxInputs));
  if (nbOutputs >= nbMaxOutputs)
    throw DYNError(Error::GENERAL, AutomatonMaximumOutputSizeReached, modelName,
                   boost::lexical_cast<std::string>(nbOutputs),
                   boost::lexical_cast<std::string>(nbMaxOutputs));
  static std::string separator = ";";
  std::string workingDir = workingDirectory + "/execution/" + modelName + "/";
  if (!exists(workingDir)) create_directory(workingDir);
  ...
  for (int i = 0; i < nbOutputs; ++i) {
    std::map<std::string, double>::const_iterator iter =
        values.find(outputsName[i]);
    if (iter == values.end())
      throw DYNError(Error::GENERAL, UnknownAutomatonOutput, modelName,
                     outputsName[i]);
    outputs[i] = iter->second;
  }
}

void initHelicsCosimulationInterface(ModelManager* manager,
                                     const std::string& workingDirectory) {
  std::shared_ptr<helicscpp::ValueFederate> fed =
      std::make_shared<helicscpp::ValueFederate>(workingDirectory +
                                                 "/Dynawo.json");
...
std::string getLocalHelicsPubName(std::string pubName) {
  ...
  if (npos != std::string::npos) return pubName.substr(npos + 1);
  return pubName;
}

void callHelicsCosimulationInterfaceModel(
    ModelManager* manager, const std::string& modelName, const double time,
    const double* inputs, const char** inputsName, const int nbInputs,
    const int nbMaxInputs, double* outputs, const char** outputsName,
    const int nbOutputs, const int nbMaxOutputs,
    const std::string& workingDirectory) {
  if (nbInputs >= nbMaxInputs)
    throw DYNError(Error::GENERAL, AutomatonMaximumInputSizeReached, modelName,
  ...
  // Only communicate with helics the first time the automaton is activated for
  // at a given time (not after solver reinits)
  if (time > manager->helicsTime_) {
    ...
    for (int i = 0; i < pubCount; i++) {
      ...
      while (found < nbInputs) {
        if (inputsName[found] == pubName) break;
        found++;
      }
      if (found == nbInputs) {
        throw DYNError(Error::GENERAL, UnknownAutomatonInput, modelName,
                       pubName);
      }
...
modelica_real computeDelay(ModelManager* manager, int exprNumber,
                           double exprValue, double time, double delayTime,
                           double delayMax) {
  return manager->computeDelay(exprNumber, exprValue, time, delayTime,
                               delayMax);
}

void addDelay(ModelManager* manager, int exprNumber, const double* time,
