...
DYNDATA* ModelManager::data() const {
  if (modelInitUsed_)
    return dataInit_;
  else
    return dataDyn_;
...
MODEL_DATA* ModelManager::modelData() const { return data()->modelData; }

SIMULATION_INFO* ModelManager::simulationInfo() const {
  return data()->simulationInfo;
}

ModelModelica* ModelManager::modelModelicaInit() const {
  if (hasInit())
    return modelInit_;
  else
...
ModelModelica* ModelManager::modelModelicaDynamic() const { return modelDyn_; }

ModelModelica* ModelManager::modelModelica() const {
...
void ModelManager::initializeStaticData() {
  if (hasInit()) modelInit_->initData(dataInit_);

  modelDyn_->initData(dataDyn_);
}

void ModelManager::createParametersValueSet(
    const boost::unordered_map<string, ParameterModeler>& parameters,
    shared_ptr<ParametersSet>& parametersSet) {
  for (ParamIterator it = parameters.begin(), itEnd = parameters.end();
       it != itEnd; ++it) {
    ...
    if (parameter.hasValue()) {
      switch (parameter.getValueType()) {
        case VAR_TYPE_DOUBLE: {
          parametersSet->createParameter(parameterName,
...
void ModelManager::initSubBuffers() { associateBuffers(); }

void ModelManager::init(const double t0) {
...
void ModelManager::setSubModelParameters() {
  if (modelModelica()->isDataStructInitialized()) {
    shared_ptr<ParametersSet> mergedParametersSet(
        boost::shared_ptr<ParametersSet>(
            new ParametersSet("merged_" + name())));

    const boost::unordered_map<string, ParameterModeler>& parameters =
        getParametersDynamic();
...
void ModelManager::associateBuffers() {
  if (modelInitUsed_) {
    yLocalInit_.resize(dataInit_->nbVars);
    ypLocalInit_.resize(dataInit_->nbVars);
    zLocalInit_.resize(dataInit_->nbZ +
                       dataInit_->modelData->nVariablesInteger);
...
void ModelManager::getSize() {
  sizeF_ = data()->nbF;
  sizeZ_ = static_cast<unsigned int>(
      data()->nbZ +
      modelData()->nVariablesInteger);  ///< Z in dynawo = Z in Modelica + I in
                                        ///< Modelica
  sizeG_ =
      static_cast<unsigned int>(modelData()->nZeroCrossings + data()->nbDelays);
...
void ModelManager::evalF(double t, propertyF_t type) {
...
bool ModelManager::hasDataCheckCoherence() const {
  return modelModelica()->hasCheckDataCoherence();
}

void ModelManager::checkDataCoherence(const double t) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
...
void ModelManager::checkParametersCoherence() const {
  modelModelica()->checkParametersCoherence();
}

void ModelManager::setFequations() {
  modelModelicaDynamic()->setFequations(fEquationIndex_);
}

void ModelManager::setGequations() {
  modelModelicaDynamic()->setGequations(gEquationIndex_);
}

void ModelManager::setFequationsInit() {
  if (hasInit()) modelModelicaInit()->setFequations(fEquationInitIndex_);
}

void ModelManager::setGequationsInit() {
  if (hasInit()) modelModelicaInit()->setGequations(gEquationInitIndex_);
}

#ifdef _ADEPT_

void ModelManager::evalF(const double t, const vector<adept::adouble>& y,
                         const vector<adept::adouble>& yp,
                         vector<adept::adouble>& f) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  ...
  for (unsigned int i = 0; i < sizeF(); i++) {
    ...
    if (isnan(term) || isinf(term)) {
      throw DYNError(Error::MODELER, ResidualWithNanInf, name(), modelType(),
                     staticId(), i,
                     getFequationByLocalIndex(i));  // i is local index
    }
  }
#endif
}

void ModelManager::evalJtAdept(const double t, double* y, double* yp,
                               const double cj, SparseMatrix& Jt,
                               const int rowOffset, bool complete) {
  if (sizeY() == 0) return;
  ...
  try {
    ...
    const int coeff =
        complete ? 1 : 0;  // complete => jacobian @F/@y + cj.@F/@Y' else @F/@Y'
...
#ifdef _DEBUG_
        if (isnan(term) || isinf(term)) {
          throw DYNError(Error::MODELER, JacobianWithNanInf, name(),
                         modelType(), staticId(), i,
                         getFequationByLocalIndex(i), j);  // i is local index
        }
#endif
...
void ModelManager::evalG(const double t) {
  setManagerTime(t);
...
void ModelManager::evalJt(const double t, const double cj, SparseMatrix& jt,
                          const int rowOffset) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
...
void ModelManager::evalJtPrim(const double t, const double cj, SparseMatrix& jt,
                              const int rowOffset) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
...
void ModelManager::evalZ(const double t) {
  if (sizeZ() > 0) {
    setManagerTime(t);
...
modeChangeType_t ModelManager::evalMode(const double t) {
  modeChangeType_t delay_mode =
      delayManager_.isTriggered() ? ALGEBRAIC_MODE : NO_MODE;
  if (delayManager_.isTriggered()) {
    // reset trigger if delay mode is detected to prevent detection next time
...
void ModelManager::evalStaticYType() {
  modelModelica()->evalStaticYType_omc(yType_);
}

void ModelManager::evalStaticFType() {
  modelModelica()->evalStaticFType_omc(fType_);
}

void ModelManager::evalDynamicYType() {
  modelModelica()->evalDynamicYType_omc(yType_);
}

void ModelManager::evalDynamicFType() {
  modelModelica()->evalDynamicFType_omc(fType_);
}

void ModelManager::collectSilentZ(BitMask* silentZTable) {
  modelModelica()->collectSilentZ(silentZTable);
}

void ModelManager::setSharedParametersDefaultValues(
    const bool isInit, const parameterOrigin_t& origin) {
  ModelModelica* model = isInit ? modelModelicaInit() : modelModelicaDynamic();
  const shared_ptr<parameters::ParametersSet> sharedParametersInitialValues =
      model->setSharedParametersDefaultValues();
  const boost::unordered_map<string, ParameterModeler>& parameters =
      isInit ? getParametersInit() : getParametersDynamic();
  ...
  for (ParamIterator it = parameters.begin(), itEnd = parameters.end();
    ...
    if (currentParameter.isUnitary() &&
        sharedParametersInitialValues->hasParameter(paramName)) {
      switch (currentParameter.getValueType()) {
        case VAR_TYPE_BOOL: {
          const bool value =
              sharedParametersInitialValues->getParameter(paramName)->getBool();
          setParameterValue(paramName, origin, value, isInit);
          break;
        }
        case VAR_TYPE_INT: {
          const int value =
              sharedParametersInitialValues->getParameter(paramName)->getInt();
          setParameterValue(paramName, origin, value, isInit);
          break;
        }
        case VAR_TYPE_DOUBLE: {
          const double& value =
              sharedParametersInitialValues->getParameter(paramName)
                  ->getDouble();
          setParameterValue(paramName, origin, value, isInit);
          break;
        }
...
void ModelManager::initParams() {
  ...
  shared_ptr<ParametersSet> mergedParametersSet(
      boost::shared_ptr<ParametersSet>(new ParametersSet("merged_" + name())));
  const boost::unordered_map<string, ParameterModeler>& parametersInit =
      getParametersInit();
...
void ModelManager::dumpParameters(map<string, string>& mapParameters) {
  ...
  std::copy(simulationInfo()->realParameter,
            simulationInfo()->realParameter + nb, params.begin());

  nb = static_cast<unsigned int>(modelData()->nParametersBoolean);
  vector<bool> paramsBool(nb, false);
  std::copy(simulationInfo()->booleanParameter,
            simulationInfo()->booleanParameter + nb, paramsBool.begin());

  nb = static_cast<unsigned int>(modelData()->nParametersInteger);
  vector<int> paramsInt(nb, 0);
  std::copy(simulationInfo()->integerParameter,
            simulationInfo()->integerParameter + nb, paramsInt.begin());
  ...
  for (std::vector<std::string>::const_iterator it = delays.begin();
       it != delays.end(); ++it) {
...
void ModelManager::writeParametersFinalValues() {
  ...
  const unsigned int nbParamsReal =
      static_cast<unsigned int>(modelData()->nParametersReal);
  const unsigned int nbParamsBool =
      static_cast<unsigned int>(modelData()->nParametersBoolean);
  const unsigned int nbParamsInt =
      static_cast<unsigned int>(modelData()->nParametersInteger);
  for (ParamIterator it = parameters.begin(), itEnd = parameters.end();
       it != itEnd; ++it) {
    const ParameterModeler& currentParameter = it->second;
    ...
    if (i < nbParamsReal) {
      ...
      switch (currentParameter.getValueType()) {
        case VAR_TYPE_DOUBLE: {
          setFinalParameter(currentParameterName,
                            simulationInfo()->realParameter[localRealIndex]);
          break;
        }
        case VAR_TYPE_INT: {
          setFinalParameter(
        ...
        case VAR_TYPE_BOOL: {
          setFinalParameter(
              currentParameterName,
              toNativeBool(simulationInfo()->realParameter[localRealIndex]));
          break;
        }
        case VAR_TYPE_STRING: {
          throw DYNError(Error::MODELER, ParameterInvalidTypeRequested,
                         currentParameter.getName(),
                         typeVarC2Str(currentParameter.getValueType()),
                         "DOUBLE");
        }
      }
      ...
      if (currentParameter.getValueType() != VAR_TYPE_BOOL)
        throw DYNError(Error::MODELER, ParameterInvalidTypeRequested,
                       currentParameter.getName(),
                       typeVarC2Str(currentParameter.getValueType()), "BOOL");

      setFinalParameter(currentParameterName,
                        simulationInfo()->booleanParameter[localBooleanIndex]);
      ...
      if (currentParameter.getValueType() != VAR_TYPE_INT)
        throw DYNError(Error::MODELER, ParameterInvalidTypeRequested,
                       currentParameter.getName(),
                       typeVarC2Str(currentParameter.getValueType()), "INT");

      setFinalParameter(currentParameterName,
                        simulationInfo()->integerParameter[localIntegerIndex]);
    }
  }
}

void ModelManager::getSubModelParameterValue(const string& nameParameter,
  ...
  if (found) {
    ...
    // check that the final parameters values have been filled : if not, fill
    // them
...
void ModelManager::dumpVariables(map<string, string>& mapVariables) {
  stringstream values;
  ...
  std::copy(data()->localData[0]->realVars, data()->localData[0]->realVars + nb,
            valuesReal.begin());

  vector<double> valuesDerivatives(nb, 0.);
  std::copy(data()->localData[0]->derivativesVars,
            data()->localData[0]->derivativesVars + nb,
            valuesDerivatives.begin());

  nb = static_cast<unsigned int>(modelData()->nVariablesBoolean);
  vector<bool> valuesBool(nb, false);
  std::copy(data()->localData[0]->booleanVars,
            data()->localData[0]->booleanVars + nb, valuesBool.begin());

  nb = static_cast<unsigned int>(modelData()->nVariablesInteger);
  vector<double> valuesInt(nb, 0);
  std::copy(data()->localData[0]->integerDoubleVars,
            data()->localData[0]->integerDoubleVars + nb, valuesInt.begin());

  nb = data()->nbZ;
  vector<double> valuesDiscreteReal(nb, 0.);
  std::copy(data()->localData[0]->discreteVars,
            data()->localData[0]->discreteVars + nb,
            valuesDiscreteReal.begin());

  vector<double> constCalcVars(data()->constCalcVars.size(), 0.);
  std::copy(data()->constCalcVars.begin(), data()->constCalcVars.end(),
            constCalcVars.begin());
...
void ModelManager::loadVariables(const string& variables) {
  setManagerTime(getCurrentTime());
  ...
  if (static_cast<unsigned>(modelData()->nVariablesBoolean) !=
      valuesBool.size())
    throw DYNError(Error::MODELER, WrongDataNum, variablesFileName().c_str());
  ...
  std::copy(valuesReal.begin(), valuesReal.end(),
            data()->localData[0]->realVars);
  std::copy(valuesDerivatives.begin(), valuesDerivatives.end(),
            data()->localData[0]->derivativesVars);
  std::copy(valuesInt.begin(), valuesInt.end(),
            data()->localData[0]->integerDoubleVars);
  std::copy(valuesBool.begin(), valuesBool.end(),
            data()->localData[0]->booleanVars);
  std::copy(valuesDiscreteReal.begin(), valuesDiscreteReal.end(),
            data()->localData[0]->discreteVars);
  std::copy(constCalcVars.begin(), constCalcVars.end(),
            data()->constCalcVars.begin());
  std::copy(valuesRoots.begin(), valuesRoots.end(), gLocal_);
}

void ModelManager::loadParameters(const string& parameters) {
  ...
  if (parameterDoubleValues.size() !=
      static_cast<unsigned>(modelData()->nParametersReal))
    throw DYNError(Error::MODELER, WrongDataNum, parametersFileName().c_str());

  if (parameterBoolValues.size() !=
      static_cast<unsigned>(modelData()->nParametersBoolean))
    throw DYNError(Error::MODELER, WrongDataNum, parametersFileName().c_str());

  if (parameterIntValues.size() !=
      static_cast<unsigned>(modelData()->nParametersInteger))
    throw DYNError(Error::MODELER, WrongDataNum, parametersFileName().c_str());

  if (parameterStringValues.size() <
      static_cast<unsigned>(modelData()->nParametersString))
    throw DYNError(Error::MODELER, WrongDataNum, parametersFileName().c_str());

  // loading of read parameters
  for (unsigned int i = 0;
       i < static_cast<unsigned>(modelData()->nParametersString); ++i)
    simulationInfo()->stringParameter[i] = parameterStringValues[i].c_str();

  std::vector<std::string> delay_def(
      parameterStringValues.begin() +
          static_cast<unsigned>(modelData()->nParametersString),
      parameterStringValues.end());
  ...
  const boost::unordered_map<string, ParameterModeler>& parametersMap =
      (this)->getParametersDynamic();
  // We need ordered parameters as Modelica structures are ordered in a certain
  // way and we want to stick to this order to recover the param
  vector<string> parametersList(parametersMap.size(), "TMP");
  for (ParamIterator it = parametersMap.begin(), itEnd = parametersMap.end();
       it != itEnd; ++it) {
    const ParameterModeler& currentParameter = it->second;
    parametersList[currentParameter.getIndex()] = it->first;
  }
  for (unsigned int i = 0; i < modelData()->nParametersReal; ++i) {
    const ParameterModeler& currentParameter =
        parametersMap.at(parametersList[i]);
    switch (currentParameter.getValueType()) {
      case VAR_TYPE_DOUBLE: {
        setLoadedParameter(currentParameter.getName(),
                           parameterDoubleValues[i]);
        break;
      }
      case VAR_TYPE_INT: {
        setLoadedParameter(currentParameter.getName(),
                           static_cast<int>(parameterDoubleValues[i]));
        break;
      }
      case VAR_TYPE_BOOL: {
        setLoadedParameter(currentParameter.getName(),
                           toNativeBool(parameterDoubleValues[i]));
        break;
      }
      case VAR_TYPE_STRING: {
        throw DYNError(Error::MODELER, ParameterInvalidTypeRequested,
                       currentParameter.getName(),
                       typeVarC2Str(currentParameter.getValueType()), "DOUBLE");
      }
    }
  }
...
void ModelManager::solveParameters() {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  ...
  if (!yLocalInit_.empty()) y = &yLocalInit_[0];
  if (!ypLocalInit_.empty()) yp = &ypLocalInit_[0];
  if (!zLocalInit_.empty()) z = &zLocalInit_[0];
  if (!fLocalInit_.empty()) f = &fLocalInit_[0];
  setBufferY(y, yp, 0);
  setBufferZ(z, zConnected, 0);
  setBufferF(f, 0);
  getY0();

  // computation of initial jroot, allow to find the correct value initial value
  // of Z
  ...
  if (!g0.empty()) g0Safe = &g0[0];
  if (!g1.empty()) g1Safe = &g1[0];
  setBufferG(g0Safe, 0);
  ...
  // we loop until we find stable initial values (handle of dependencies Z->Y,
  // Y->Z)
  ...
  if (sizeY() != sizeF())
    throw DYNError(Error::MODELER, SolverSubModelYvsF, name(), sizeY(),
                   sizeF());
  ...
  do {
    ...
    if (!stableRoot) {
      ...
      // case of function to compute discrete variables (used to transform for
      // example)
      evalZ(t0);
    }
    ...
    ++counter;
    if (counter >= 10) throw DYNError(Error::MODELER, UnstableRoots);
  ...
  if (fErr.size() > 0) {
    unsigned i = 0;
    for (map<double, int>::const_iterator it = fErr.begin();
         it != fErr.end() && i < nbErr; ++it, ++i) {
      Trace::debug() << DYNLog(SolveParametersFError, tolerance, it->second,
                               it->first, getFequationByLocalIndex(it->second))
                     << Trace::endline;
    }
  }
#endif
...
void ModelManager::setCalculatedParameters(
    vector<double>& y, vector<double>& z,
    const vector<double>& calculatedVars) {
  // Creates reversed alias map
  map<string, vector<shared_ptr<VariableAlias> > > reversedAlias;
  for (boost::unordered_map<string, shared_ptr<Variable> >::const_iterator it =
           variablesByNameInit_.begin();
       it != variablesByNameInit_.end(); ++it) {
    // map of nativeVarName -> aliasNames
    if (it->second->isAlias()) {
      const shared_ptr<VariableAlias> variable =
          dynamic_pointer_cast<VariableAlias>(it->second);
      const string& referenceVariableName =
          variable->getReferenceVariableName();
      if (reversedAlias.find(referenceVariableName) == reversedAlias.end()) {
        // First time the alias name is seen
        reversedAlias[referenceVariableName] =
            vector<shared_ptr<VariableAlias> >(1, variable);
      } else {
...
void ModelManager::setYCalculatedParameters(
    vector<double>& y,
  ...
  for (unsigned int i = 0; i < xNamesInitial.size(); ++i) {
    ...
    if (reversedAlias.find(xNamesInitial[i]) != reversedAlias.end()) {
      vector<shared_ptr<VariableAlias> > variables =
          reversedAlias.find(xNamesInitial[i])->second;
      for (vector<shared_ptr<VariableAlias> >::const_iterator it =
               variables.begin();
           it != variables.end(); ++it) {
...
void ModelManager::setZCalculatedParameters(
    vector<double>& z,
    const map<string, vector<shared_ptr<VariableAlias> > >& reversedAlias) {
  const vector<string>& zNamesInitial = zNamesInit();

  const boost::unordered_map<string, shared_ptr<Variable> >& initVariables =
      variablesByNameInit();

  assert(zNamesInitial.size() == z.size());
  for (unsigned int i = 0; i < zNamesInitial.size(); ++i) {
    bool toConvertToBool =
        false;  // whether the variable is a Modelica boolean (described through
                // discrete Real) and should be converted back to C++ boolean
    bool toConvertToInt = false;  // whether the variable is a Modelica integer
    const string& varName = zNamesInitial[i];

    // check whether the variable is an alias (in order to determine whether it
    // is a boolean variable)
    string varNameForCheck = varName;
    if ((initVariables.find(varName) == initVariables.end()) &&
        (reversedAlias.find(varName) != reversedAlias.end())) {
      vector<shared_ptr<VariableAlias> > variables =
          reversedAlias.find(varName)->second;
      for (vector<shared_ptr<VariableAlias> >::const_iterator it =
               variables.begin();
           it != variables.end(); ++it) {
    ...
    // Check whether there is a need to convert the variable to native boolean
    // or to an integer
    if (initVariables.find(varNameForCheck) != initVariables.end()) {
      const shared_ptr<Variable> var =
          initVariables.find(varNameForCheck)->second;
      toConvertToBool = (var->getType() == BOOLEAN);
      toConvertToInt = (var->getType() == INTEGER);
    }

    // a ternary operator does not work here, because the types of
    // toNativeBool(z[i]) and z[i] are different as a result, toNativeBool(z[i])
    // would be cast as a double, leading to a downstream error ("expected bool,
    // got double")
    if (toConvertToBool) {
...
void ModelManager::setInitialCalculatedParameters() {
  const boost::unordered_map<string, ParameterModeler>& parametersMap =
      getParametersInit();
  // We need ordered parameters as Modelica structures are ordered in a certain
  // way and we want to stick to this order to recover the param
  ...
  assert(parametersInitial.size() ==
         static_cast<size_t>(
             modelData()->nParametersReal + modelData()->nParametersBoolean +
  ...
  for (unsigned int i = 0; i < modelData()->nParametersReal; ++i) {
    ...
    if (hasParameterDynamic(parName)) {
      ...
      if (!parameter.isFullyInternal()) {
        // ternary operator does not work here (because the boolean would be
        // implicitly converted to double, leading to a downstream parameter
        // type error)
        switch (parameter.getValueType()) {
          case VAR_TYPE_DOUBLE: {
          ...
          case VAR_TYPE_INT: {
            setCalculatedParameter(
                parName, static_cast<int>(simulationInfo()->realParameter[i]));
            break;
          }
          case VAR_TYPE_BOOL: {
            setCalculatedParameter(
                parName, toNativeBool(simulationInfo()->realParameter[i]));
            break;
          }
          case VAR_TYPE_STRING: {
            throw DYNError(Error::MODELER, ParameterInvalidTypeRequested,
                           parName, typeVarC2Str(parameter.getValueType()),
                           "DOUBLE");
          }
        }
      }
  ...
  for (unsigned int i = 0; i < modelData()->nParametersBoolean; ++i) {
    ...
    if (hasParameterDynamic(parName)) {
      const ParameterModeler& parameter = findParameterDynamic(parName);
      if (!parameter.isFullyInternal()) {
        setCalculatedParameter(parName,
                               simulationInfo()->booleanParameter[i] > 0);
      }
    }
  }
  ...
  for (unsigned int i = 0; i < modelData()->nParametersInteger; ++i) {
    ...
    if (hasParameterDynamic(parName)) {
      ...
      if (!parameter.isFullyInternal()) {
        setCalculatedParameter(
            parName, static_cast<int>(simulationInfo()->integerParameter[i]));
      }
    }
  }
  ...
  for (unsigned int i = 0; i < modelData()->nParametersString; ++i) {
    ...
    if (hasParameterDynamic(parName)) {
      ...
      if (!parameter.isFullyInternal()) {
        setCalculatedParameter(
            parName, std::string(simulationInfo()->stringParameter[i]));
      }
    }
  }
}

void ModelManager::createCalculatedParametersFromInitialCalculatedVariables(
    const vector<double>& calculatedVars,
    const map<string, vector<shared_ptr<VariableAlias> > >& reversedAlias) {
  ...
  for (unsigned int i = 0; i < calcVarInitial.size(); ++i) {
    ...
    if (reversedAlias.find(calcVarInitial[i]) != reversedAlias.end()) {
      vector<shared_ptr<VariableAlias> > variables =
          reversedAlias.find(calcVarInitial[i])->second;
      for (vector<shared_ptr<VariableAlias> >::const_iterator it =
               variables.begin();
           it != variables.end(); ++it) {
        if (!(*it)->getNegated()) {  // Usual alias
...
void ModelManager::printValuesParameters(std::ofstream& fstream) {
  ...
  // In Modelica models, parameters are ordered as follows : real, then boolean,
  // integer and string
  for (unsigned int i = 0; i < modelData()->nParametersReal; ++i)
    fstream << std::setw(50) << std::left << parameters[i] << std::right << " ="
            << std::setw(15)
  ...
  for (unsigned int i = 0; i < modelData()->nParametersBoolean; ++i)
    fstream << std::setw(50) << std::left << parameters[i + offset]
            << std::right << " =" << std::setw(15) << std::boolalpha
...
string ModelManager::modelType() const { return modelType_; }

void ModelManager::rotateBuffers() {
  // copy data()->localData[0]->realVars -> simulationInfo()->realVarsPre
  if (modelData()->nVariablesReal > 0)
    memcpy(simulationInfo()->realVarsPre, data()->localData[0]->realVars,
           sizeof(data()->localData[0]->realVars[0]) *
               modelData()->nVariablesReal);

  // copy data()->localData[0]->booleanVars -> simulationInfo()->booleanVarsPre
  if (modelData()->nVariablesBoolean > 0)
    memcpy(simulationInfo()->booleanVarsPre, data()->localData[0]->booleanVars,
           sizeof(data()->localData[0]->booleanVars[0]) *
               modelData()->nVariablesBoolean);

  // copy  data()->localData[0]->discreteVars ->
  // simulationInfo()->discreteVarsPre
  if (data()->nbZ > 0)
    memcpy(simulationInfo()->discreteVarsPre,
           data()->localData[0]->discreteVars,
           sizeof(data()->localData[0]->discreteVars[0]) * data()->nbZ);

  if (modelData()->nVariablesInteger > 0)
    memcpy(simulationInfo()->integerDoubleVarsPre,
           data()->localData[0]->integerDoubleVars,
           sizeof(data()->localData[0]->integerDoubleVars[0]) *
               modelData()->nVariablesInteger);

  if (modelData()->nRelations > 0)
    memcpy(simulationInfo()->relationsPre, simulationInfo()->relations,
           sizeof(simulationInfo()->relations[0]) * modelData()->nRelations);
}

void ModelManager::defineVariablesInit(
    vector<shared_ptr<Variable> >& variables) {
  if (!hasInit()) return;
  modelInit_->defineVariables(variables);
}

void ModelManager::defineParametersInit(vector<ParameterModeler>& parameters) {
  if (!hasInit()) return;

  modelInit_->defineParameters(parameters);
}

void ModelManager::defineVariables(vector<shared_ptr<Variable> >& variables) {
  modelDyn_->defineVariables(variables);
}

void ModelManager::defineParameters(vector<ParameterModeler>& parameters) {
  modelDyn_->defineParameters(parameters);
}

void ModelManager::defineElements(vector<Element>& elements,
                                  map<string, int>& mapElement) {
  modelDyn_->defineElements(elements, mapElement);
}

void ModelManager::setManagerTime(const double st) {
  data()->localData[0]->timeValue = st;
}

void ModelManager::notifyTimeStep() { delayManager_.saveTimepoint(); }

void ModelManager::evalCalculatedVars() {
  modelModelica()->evalCalculatedVars(calculatedVars_);
}

double ModelManager::evalCalculatedVarI(unsigned iCalculatedVar) const {
  return modelModelica()->evalCalculatedVarI(iCalculatedVar);
}

void ModelManager::evalJCalculatedVarI(unsigned iCalculatedVar,
  ...
  try {
    ...
    stack.new_recording();
    adept::adouble output =
        modelModelica()->evalCalculatedVarIAdept(iCalculatedVar, 0, x, xp);
...
void ModelManager::getIndexesOfVariablesUsedForCalculatedVarI(
    unsigned iCalculatedVar, std::vector<int>& indexes) const {
  return modelModelica()->getIndexesOfVariablesUsedForCalculatedVarI(
      iCalculatedVar, indexes);
}

void ModelManager::initializeFromData(
    const boost::shared_ptr<DataInterface>& /*data*/) {
  // not needed
}

void ModelManager::addDelay(int exprNumber, const double* time,
                            const double* exprValue, double delayMax) {
  delayManager_.addDelay(exprNumber, time, exprValue, delayMax);
}

double ModelManager::computeDelay(int exprNumber, double exprValue, double time,
                                  double delayTime, double delayMax) {
  if (delayTime > delayMax || delayTime < 0.0) {
    throw DYNError(DYN::Error::SIMULATION, IncorrectDelay, delayTime, time,
                   delayMax);
  }

#if _DEBUG_
  // this shouldn't happen by construction of the model files generated by
  // scripts
  ...
  if (time < delayTime || doubleEquals(time, delayTime)) {
    // requested time is allowed but the simulation has not run for enought time
    // to compute the delayed value: use the initial value to avoid artefacts
    const boost::optional<double>& initialValue =
        delayManager_.getInitialValue(exprNumber);
#if _DEBUG_
