 ...
 *
 * @brief Class of Model Manager : interface between a modelica model and a
 * subModel
 *
 */
...
class ModelManager : public SubModel, private boost::noncopyable {
 public:
  ...
  /**
   * @copydoc SubModel::evalJt(const double t,const double cj, SparseMatrix& Jt,
   * const int rowOffset)
   */
  ...
  /**
   * @copydoc SubModel::evalJtPrim(const double t,const double cj, SparseMatrix&
   * Jt, const int rowOffset)
   */
  void evalJtPrim(const double t, const double cj, SparseMatrix& Jt,
                  const int rowOffset);
  /**
  ...
  /**
   * @copydoc SubModel::dumpParameters(std::map< std::string, std::string > &
   * mapParameters)
   */
  void dumpParameters(std::map<std::string, std::string>& mapParameters);

  /**
   * @copydoc SubModel::dumpVariables(std::map< std::string, std::string > &
   * mapVariables)
   */
  void dumpVariables(std::map<std::string, std::string>& mapVariables);

  /**
   * @brief write the final parameters values (relying on low level Modelica
   * data structures)
   * @throw when the parameter value type in Modelica data structures is
   * incompatible with the value type in Dynawo Parameter
   */
  void writeParametersFinalValues();

  /**
   * @copydoc SubModel::getSubModelParameterValue(const std::string &
   * nameParameter, std::string& value, bool & found)
   */
  void getSubModelParameterValue(const std::string& nameParameter,
  ...
  /**
   * @copydoc SubModel::defineElements(std::vector<Element> &elements,
   * std::map<std::string, int>& mapElement)
   */
  void defineElements(std::vector<Element>& elements,
                      std::map<std::string, int>& mapElement);
   ...
   * @return value of the calculated variable based on the current values of
   * continuous variables
   */
  double evalCalculatedVarI(unsigned iCalculatedVar) const;

  /**
   * @brief evaluate the jacobian associated to a calculated variable based on
   * the current values of continuous variables
   *
   * @param iCalculatedVar index of the calculated variable
   * @param res values of the jacobian
   */
  void evalJCalculatedVarI(unsigned iCalculatedVar,
   ...
   *
   */
  void getIndexesOfVariablesUsedForCalculatedVarI(
  ...
  /**
   * @copydoc SubModel::defineVariables(std::vector<boost::shared_ptr<Variable>
   * >& variables)
   */
  void defineVariables(std::vector<boost::shared_ptr<Variable> >& variables);

  /**
   * @copydoc SubModel::defineParameters(std::vector<ParameterModeler>&
   * parameters)
   */
  ...
  /**
   * @copydoc
   * SubModel::defineVariablesInit(std::vector<boost::shared_ptr<Variable> >&
   * variables)
   */
  ...
  /**
   * @copydoc SubModel::defineParametersInit(std::vector<ParameterModeler>&
   * parameters)
   */
  void defineParametersInit(std::vector<ParameterModeler>& parameters);

  /**
   * @copydoc SubModel::initializeFromData(const
   ...
   */
  void setSharedParametersDefaultValues(const bool isInit,
   ...
   *
   * Retrieves the value corresponding to timepoint @p time - @p delayTime if
   * allowed by @p delayMax
   *
   * @throw IncorrectDelay error if @p delayTime is not compatible (negative or
   * less than @p delayMax)
   *
   * @param exprValue the value corresponding to @p time
   * @param exprNumber the id of the delay, in practice the index in the arrays
   ...
   */
  double computeDelay(int exprNumber, double exprValue, double time,
   ...
   * @param exprValue pointer to the value that will be externally updated at
   * runtime
   * @param delayMax maximum allowed delay
   */
  void addDelay(int exprNumber, const double* time, const double* exprValue,
                double delayMax);
 ...
 private:
   ...
   */
  void evalF(const double t, const std::vector<adept::adouble>& y,
             const std::vector<adept::adouble>& yp,
             std::vector<adept::adouble>& f);
   ...
   * @param complete @b true if \f$( J=@F/@x + cj * @F/@x')\f$ else \f$( J =
   * @F/@x')\f$
   */
  void evalJtAdept(const double t, double* y, double* yp, const double cj,
                   SparseMatrix& Jt, const int rowOffset, bool complete = true);
#endif
  ...
  void setCalculatedParameters(std::vector<double>& y, std::vector<double>& z,
                               const std::vector<double>& calculatedVars);
   ...
   */
  inline void setCalculatedParameter(const std::string& name,
                                     const double value) {
    setParameterValue(name, LOCAL_INIT, value, false);
  }

  /**
   * @copydoc ModelManager::setCalculatedParameter (const std::string& name,
   * const double& value)
   */
  ...
  /**
   * @copydoc ModelManager::setFinalParameter (const std::string& name, const
   * double& value)
   */
   ...
   * double& value)
   */
  inline void setFinalParameter(const std::string& name,
                                const std::string& value) {
    setParameterValue(name, FINAL, value, false);
  }
  ...
  /**
   * @copydoc ModelManager::setLoadedParameter (const std::string& name, const
   * double& value)
   */
   ...
   * double& value)
   */
  inline void setLoadedParameter(const std::string& name,
                                 const std::string value) {
    setParameterValue(name, LOADED_DUMP, value, false);
  }
 ...
 private:
   ...
   */
  void createParametersValueSet(
      const boost::unordered_map<std::string, ParameterModeler>& parameters,
 ...
 private:
   ...
   *
   * if the current model used is the init model, return the dynamic data for
   * the init model otherwise return the dynamic data for the standard model
   ...
   *
   * if the current model used is the init model, return the model data for the
   * init model otherwise return the model data for the standard model
   ...
   *
   * if the current model used is the init model, return the simulation infos
   * for the init model otherwise return the simulation info for the standard
   * model
   ...
   */
  void setYCalculatedParameters(
      std::vector<double>& y,
      const std::map<std::string,
                     std::vector<boost::shared_ptr<VariableAlias> > >&
          reversedAlias);
   ...
   * @param reversedAlias reference variable name to alias variables map
   */
  void createCalculatedParametersFromInitialCalculatedVariables(
      const std::vector<double>& calculatedVars,
      const std::map<std::string,
                     std::vector<boost::shared_ptr<VariableAlias> > >&
          reversedAlias);
 ...
 private:
   ...
   */
  typedef boost::unordered_map<std::string, ParameterModeler>::const_iterator
      ParamIterator;
